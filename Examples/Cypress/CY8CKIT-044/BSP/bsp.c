                                ...
                                CPU_INT08U          *value)
{
    ...
    i2c_status = I2C_I2CMasterSendStart(BSP_I2C_KXTJ2_ACC_SLV_ADDR, BSP_KXTJ2_DIR_WRITE);
    if (i2c_status == I2C_I2C_MSTR_NO_ERROR) {                  /* Sequence Start Successful, Perform Read/Write Action.*/
        i2c_status = I2C_I2CMasterWriteByte(reg);               /* Select Register Address to Read/Write to.            */
        
        if (rd_wr == BSP_KXTJ2_DIR_WRITE) {                     /* -------------------- WRITE DATA -------------------- */
            i2c_status   = I2C_I2CMasterWriteByte(*value);      /* Write Data Byte to the Desired Register.             */
            accel_status = DEF_OK;
    ...
    i2c_status = I2C_I2CMasterSendStop();                       /* Send the Stop Condition to Stop I2C Communication.   */
